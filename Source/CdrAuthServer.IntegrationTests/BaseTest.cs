...
namespace CdrAuthServer.IntegrationTests
{
    ...
    abstract public class BaseTest
    {
        ...
        public const string ADDITIONAL_CERTIFICATE_FILENAME = "Certificates/client-additional.pfx";
        ...
        // Software Product
        public const string SOFTWAREPRODUCT_ID = "c6327f87-687a-4369-99a4-eaacd3bb8210";  // FIXME - MJS - Update tests to lookup ClientId for a SoftwareProductId (where applicable)
        public const string GUID_FOO = "f00f00f0-f00f-f00f-f00f-f00f00f00f00";
        ...
        {
            INVALID_FOO, INVALID_EMPTY, INVALID_OMIT,
            KAMILLA_SMITH,
        }
                ...
                case TokenType.KAMILLA_SMITH:
                    {
                        (var authCode, _) = await new DataHolder_Authorise_APIv2
                        {
                            ...
                            Scope = scope,
                            RequestUri = await PAR_GetRequestUri(scope: scope) 
                        }.Authorise();
        ...
        public static async Task<DataHolder_Token_API.Response> GetToken(
            ...
            int sharingDuration = SHARING_DURATION,
            ...
            // string responseType = "code id_token")
        {
            ...
            (var authCode, _) = await new DataHolder_Authorise_APIv2
            {
                ...
                RequestUri = requestUri ?? await PAR_GetRequestUri(scope: SCOPE, sharingDuration: sharingDuration)
            }.Authorise();
        ...
        {            
            ...
            (var idColumnName, var statusColumnName) = table switch
            {
                Table.LEGALENTITY => ("legalentityid", "legalentitystatus"),
                Table.BRAND => ("brandid", "brandstatus"),
                Table.SOFTWAREPRODUCT => ("softwareproductid", "status"),
                _ => throw new NotSupportedException($"table={table.ToString()}")
            };

            using var selectCommand = new SqlCommand(@$"select {statusColumnName} from softwareproducts where softwareproductid = @softwareProductId and {idColumnName} = @idValue", connection);
            selectCommand.Parameters.AddWithValue("@softwareProductId", SOFTWAREPRODUCT_ID);
            selectCommand.Parameters.AddWithValue("@idValue", id);
        ...
        static protected void SetStatus(Table table, string id, string status)
        {
            ...
            using var updateCommand = new SqlCommand($"update softwareproducts set {statusColumnName} = @status where softwareproductid = @softwareProductId and {idColumnName} = @id", connection);
            updateCommand.Parameters.AddWithValue("@softwareProductId", SOFTWAREPRODUCT_ID);
