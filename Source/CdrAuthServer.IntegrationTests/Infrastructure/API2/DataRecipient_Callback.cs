...
namespace CdrAuthServer.IntegrationTests.Infrastructure.API2
{
    ...
    public class TestDataRecipientConsentCallback
    {
        ...
        [Fact]
        public async Task Test_CallbackWithPost()
        {
            ...
            try
            {
                ...
                callbackResponse?.method.Should().Be(HttpMethod.Post);                
        ...
        [Fact]
        public async Task Test_CallbackWithGet()
        {
            var callback = new DataRecipientConsentCallback();
            callback.Start();
            try
            {
                ...
                var callbackResponse = await callback.WaitForCallback(10);
                response.StatusCode.Should().Be(HttpStatusCode.OK);
                callbackResponse?.method.Should().Be(HttpMethod.Get);
                callbackResponse?.received.Should().Be(true);
            }
            finally
            {
        ...
        [Fact]
        public async Task Test_CallbackWithGetAndFragment()
        {
            var callback = new DataRecipientConsentCallback();
            callback.Start();
            try
            {
                ...
                var response = await httpClient.GetAsync(callback.RedirectUrl + "#foo=bar");
                var callbackResponse = await callback.WaitForCallback(10);
                response.StatusCode.Should().Be(HttpStatusCode.OK);
                callbackResponse?.method.Should().Be(HttpMethod.Get);
                callbackResponse?.received.Should().Be(true);
        ...
        [Fact]
        public async Task Test_CallbackWithGetAndQuery()
        {
            var callback = new DataRecipientConsentCallback();
            callback.Start();
            try
