...
namespace CdrAuthServer.IntegrationTests.Infrastructure.API2
{
    public class DataHolder_Token_API
    {
            ...
            else
            {
                if (useClientAssertion == true)
                {
                    var clientAssertion = new PrivateKeyJwt2
                    {
                        ...
                        Issuer = omitIssuer ? "" : (clientId ?? throw new NullReferenceException(nameof(clientId))),
                        RequireIssuer = !omitIssuer,
        ...
        static public async Task<string?> GetAccessToken(string authCode)
        {
            ...
            {
                throw new Exception($"{nameof(DataHolder_Token_API)}.{nameof(GetAccessToken)} - Error getting access token - {responseMessage.StatusCode} - {await responseMessage.Content.ReadAsStringAsync()}");
            }
