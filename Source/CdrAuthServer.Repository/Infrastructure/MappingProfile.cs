namespace CdrAuthServer.Repository.Infrastructure
{
    ...
    {
        const char multiStringSeparator = ';';
        ...
        public static void MapEntityClientClaimsToDomainClient(Client source, Domain.Entities.Client destination)
        {
            ...
            foreach (var item in source.ClientClaims)
            {
            ...
            foreach (var item in sourceProperties)
            {
                if (!ignoreList.Contains(item.Name))
                {
                    ...
                    if (item.PropertyType == typeof(IEnumerable<string>))
                    {
                        ...
                        var joinedString = string.Join(multiStringSeparator, item.GetValue(source) as IEnumerable<string>);
                        value = joinedString;
                    }
                    ...
                    if (!string.IsNullOrEmpty(value))
                    {
                        var clientCliam = new ClientClaims
                        {
                            ...
                            Type = item.GetCustomAttribute<JsonPropertyAttribute>()?.PropertyName?? item.Name,
                            Value = value
                        };
