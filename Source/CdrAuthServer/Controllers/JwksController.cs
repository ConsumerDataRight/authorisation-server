    ...
    public class JwksController : ControllerBase
    {
        ...
        private async Task<Models.JsonWebKey> GetPS256Jwk()
        {
            ...
            var jwk = new Models.JsonWebKey()
            {
                ...
                e = e,
                x5t = securityKey.X5t,
                x5c = cert != null ? [Convert.ToBase64String(cert.RawData)] : [],
            };
            return jwk;
        }
        ...
        {
            var cert = await _config.GetES256SigningCertificate() ?? throw new InvalidOperationException("Signing certificate not found.");
            var ecdsa = cert.GetECDsaPrivateKey() ?? throw new InvalidOperationException("ECDSA private key not found.");
